cc.Class({
    ...
    {
        for(var i =0;i<n;i++){
            var pokerPrefab = cc.instantiate(DdzManager.pokerPrefabS);
            pokerPrefab.parent = this.lightcard;
            //pokerPrefab.setPositionY(14);////
            var type = data[0];
            var num  = data[1];
            var info = data[2];
            var pokerController = pokerPrefab.getComponent("DdzpokerCtrS");
            pokerController.init(DdzManager.pokersAtlasS,{num:num,type:type},info);
            if(i==1){
