...
cc.Class({
    ...
    onclickContinue:function(event,num){
        let self = this;
        if(DdzManager.rule.roomType < 2){
            if(!!DdzManager.controller.minpoint){
                let jinbi = UserCenter.getYouxibiNum();
                let baoxian = UserCenter.getYinhanbiNum();
                if(jinbi < DdzManager.controller.minpoint){
                    showLoadingAni()
                    if( jinbi+ baoxian < DdzManager.controller.minpoint){
                        hideLoadingAni()
                        showAlertBox('账户金币不足',function () {
                            self.onReturnClick();
                        })
                    }else{
                        cc.log(DdzManager.controller.minpoint,jinbi)
                        let message;
                        let qugao;
                        let qudi = DdzManager.controller.minpoint - jinbi;
                        if(DdzManager.controller.maxpoint){
                            qugao = DdzManager.controller.maxpoint - jinbi;
                        ...
                        showAlertBox(message,function () {
                            loadPrefab("hall/bank/bank",function (module) {
                                ...
                                module.getComponent('bankScript').setBank(qudi,qugao,1,function (err,data) {
                                    ...
                                    cc.log('100031',data)
                                    DdzManager.rule.sendExpend(message,function (data) {
                                        cc.log('10003',data)
                                        hideLoadingAni()
                                        if(data.code == 200){
                                            if (num == 1){
