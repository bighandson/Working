...
cc.Class({
    ...
    _playEffect : function (files,sex) {
        ...
        var url = 'resources/game/ddz/sound/effect/' + files[index];
        if (!!sex) {
            if(sex == 1){  // 男的
                url = 'resources/game/ddz/sound/boy/' + files[index];
            }else {
                url = 'resources/game/ddz/sound/girl/' + files[index];
            }
        }
    ...
    _playSKSound : function (sex,filess) {
        ...
        let url;
        if(sex == 1){  // 男的
            url = 'resources/game/ddz/sound/boy/' + filess[index];
        }else {
            url = 'resources/game/ddz/sound/girl/' + filess[index];
        }
        cc.log("------_playEffec\nsex:{0},files:{1}".format(sex,url));
        SettingMgr.playSound(url);
    }
