...
cc.Class({
    ...
    RCMD_forceGameOver:function(data){
        ...
        if(!!this._agreeDissolve){
            this._agreeDissolve.init(data.userid,data.timeSpan);
            this.nextMessage();
        }else{
            let self = this;
            cc.loader.loadRes('game/poker/CardRoom/prefab/dissolve/dissPrefab',cc.Prefab,function (err,prefab) {
                if(err){
                    cc.log(err);
                    return;
                }
                ...
                self._agreeDissolve = node.getComponent('RoomDissolveControllerPoker');
