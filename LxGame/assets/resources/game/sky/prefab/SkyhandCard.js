...
var DdzCommand = require('SkyCommand');
cc.Class({
    ...
    onLoad: function () {
       ...
       cc.loader.loadRes("game/sky/prefab/SkypokerTexture/pokers", cc.SpriteAtlas, function (err, atlas) {
        ...
        var url = 'game/sky/prefab/SkypokerSprite';
    ...
    addDiPai : function (data) {
        ...
        {
            let card = data[index];
            let pokerPrefab = cc.instantiate(SkyManager.pokerPrefab);
            pokerPrefab.scale = 0.8
            ...
            //let num  = card1[1];
            let info = card[2];
            let pokerController = pokerPrefab.getComponent("SkypokerCtr");
            pokerController.init(SkyManager.pokersAtlas,{num:num,type:type},info,index);
        }
        this.refreshPokersHandLayout();
        SkyManager.rule.msgBlocked = false;
    },
    //isXianzhi 不顯示名牌按鈕
    onShow:function(data,isPlayAni,isStartMingpai,isXianzhi){
        ...
        for(let index in data)
        {
            ...
            if(isPlayAni==1/*||isPlayAni==0*/)
            {
                ...
                pokerPrefab.runAction(cc.scaleTo(0.01,0.5,0.8));
            }else{
                pokerPrefab.scale = 0.8
            }
            //type 2,4为红的
        ...
        if(isPlayAni==1)
        {
            ...
            this.schedule(this.Pokcallback, 0.1);
        }else
        {
    ...
    Pokcallback:function()
    {
        ...
        // if (!this.isMingpai) {
        //     // this.mingpaiLabel.node.parent.active = true;
        //     if (this.pokLength % 4 == 0) {
        //         let tmpBeishu = 5 - this.pokLength / 4;
        ...
        // }
        if(!!pokers) {
            pokers.runAction(cc.spawn(cc.fadeIn(0.3),cc.scaleTo(0.07,0.8,0.8)));
        }
    ...
    handCardLayoutMove:function(event)
    {
        ...
        {
            let poker = this.handCardLayout.children[i];
            let pokerController = poker.getComponent("SkypokerCtr");
            if(startPoker != poker)
            {
    ...
    handCardLayoutEnd:function(event)
    {
        ...
        // if(this.selectedArray.length == 2&&(this.type == 2||this.type == 0||this.type==4||this.type==6))
        // {
        //     var temp = SkyManager.rule.hitGetShunZi(this.handCardLayout.children,this.selectedArray,this.type,this.length);
        //     if(temp!=0)
        //     {
        //         /*for(var j = 0 ;j<this.selectedArray.length;j++)
        //         {
        //             var slected=this.selectedArray;
        //             slected[j].getComponent("SkypokerCtr").setIsSelected(false);
        //         }
        //         this.selectedArray = [];*/
        //         var tempArr = temp[0];
        ...
        //             tempArr[i].getComponent("SkypokerCtr").setIsSelected(true);
        //         }
        //     }
        // }
        this.isCanoutCard();
    ...
    refreshPokersHandLayout:function()
    {
        ...
        if(this.isBtoS)
        {
             SkyManager.rule.sortHandPokersBToS(this.handCardLayout.children);
        }
        else{
            SkyManager.rule.sortHandPokersMToL(this.handCardLayout.children);
    ...
    showOutCardAni:function(type)
    {
        ...
        if(effectName!=''){
            cc.loader.loadRes("game/ddz/effect/"+effectName, cc.Prefab, function (err, prefab) {
                if (err) {
                    return;
                }
    ...
    refreshLayout:function(type)
    {
        ...
        var data = SkyManager.rule.getCardInfo(this.selectedArray);
        var speed = 1000;
        var tt =0;
        for(let index=0;index<data.length;index++)
        {

            //var wPos = this.selectedArray[index].convertToWorldSpaceAR(this.selectedArray[index].getPosition());


            this.selectedArray[index].removeFromParent();

            //this.selectedArray[index].active = false;
            let cards = data[index];
            let card = SkyManager.getCardTypeNum(cards);
            /*var bnode = new cc.Node();
             bnode.width = 134;
             bnode.height = 180;
             bnode.parent = this.outCardLayout;
             var oldPos = bnode.convertToNodeSpaceAR(wPos);
             cc.log('this.selectedArray[index].getPosition()',oldPos);*/
            // let pokerPrefab = cc.instantiate(SkyManager.pokerPrefabS);
            let pokerPrefab = cc.instantiate(SkyManager.pokerPrefab);
            pokerPrefab.scale = 0.8
    ...
    onClickOutCard:function()
    {
        if(this.selectedArray.length!=0)
        {
            ...
            if(cardType>0)
            {
                cc.log('11111111111111',SkyManager.rule.getCardInfo(this.selectedArray));
                let array=SkyManager.rule.racklePokers(this.selectedArray,cardType);
                var msg = {
    ...
    onClickDefault : function(){
        cc.find('controller',this.node.getParent().getParent().getParent()).getComponent('SkyGame').hidMenu()
        for(let i = 0; i < this.handCardLayout.childrenCount;i++)
        {
            this.handCardLayout.children[i].getComponent("SkypokerCtr").setIsSelected(false);
        }
        for(let i = 0;i < this.selectedArray.length;i++)
        {
            let poker = this.selectedArray[i];
    ...
    onClickTips:function()
    {
        ...
        if(this.typeArray != null && this.typeArray.length > 0)
        {
            ...
            for(let i = 0;i < this.selectedArray.length;i++)
            {
                this.selectedArray[i].getComponent("SkypokerCtr").setIsSelected(true);
            }
    ...
    getTipsCardType:function(type,num,MinNum,isKtips,Rtype)
    {
        ...
        {
            case 0:
                return SkyManager.rule.getFirstPok(this.handCardLayout.children);
            case 1:
                return SkyManager.rule.getDanzhang(this.handCardLayout.children,num,MinNum,isKtips);
            case 2:
                return SkyManager.rule.getDuizi(this.handCardLayout.children,num,MinNum,true);
            case 3:
                return SkyManager.rule.getSanTiao(this.handCardLayout.children,num,MinNum,true);
            case 4:
                return SkyManager.rule.getSanDaiYidui(this.handCardLayout.children,num,MinNum);
            case 5:
                return SkyManager.rule.getShunZi(this.handCardLayout.children,num,MinNum,true);
            case 6:
                return SkyManager.rule.getLiandui(this.handCardLayout.children,num,MinNum);
            case 7:
                return SkyManager.rule.getLianSan(this.handCardLayout.children,num,MinNum,true);
            case 8:
                return SkyManager.rule.getFeiJiDaiErDui(this.handCardLayout.children,num,MinNum);//333444555
            case 9:
                return SkyManager.rule.getZhaDan(this.handCardLayout.children,4,MinNum,true);
            case 10:
                return SkyManager.rule.getZhaDan(this.handCardLayout.children,5,MinNum,true);
            case 11:
                return SkyManager.rule.getZhaDan(this.handCardLayout.children,6,MinNum,true);
            case 12:
                return SkyManager.rule.getZhaDan(this.handCardLayout.children,7,MinNum,true);
            case 13:
                return SkyManager.rule.getZhaDan(this.handCardLayout.children,8,MinNum,true);
            case 14:
                return SkyManager.rule.getWangZha(this.handCardLayout.children);
    ...
    getTipsArray:function(type,length,minNum)
    {
        ...
        if (type == 0) {
            ...
            for (var j = 9; j < 14; j++) 
            {
                let Array2=this.getTipsCardType(j,0,2,true,j);
                if(Array2.length>0)
                {
            ...
            let Array3=this.getTipsCardType(14,4,2,true,type);
            if(Array3.length>0)
            {
            ...
            for (var j = type+1; j < 15; j++) {
                let Array2=this.getTipsCardType(j,length,2,true,type);
                if(Array2.length>0)
                {
                    for(var i=0;i<Array2.length;i++)
                    {
    ...
    judgeISOUTCARD:function(type,selectArray,length,MinNum,Rtype)
    {
        ...
        {
            case 1:
            return SkyManager.rule.judgeDanZhang(selectArray,1,MinNum);break;
            case 2:
                return SkyManager.rule.judgeDuizi(selectArray,2,MinNum);break;
            case 3:
                return SkyManager.rule.judgeSanTiao(selectArray,3,MinNum);break;
            case 4:
                return SkyManager.rule.judgeSanDaiYiDui(selectArray,5,MinNum);break;
            case 5:
                return SkyManager.rule.judgeShunzi(selectArray,selectArray,length,MinNum);break;
            case 6:
                return SkyManager.rule.judgeLianDui(selectArray,length,MinNum);break;
            case 7:
                return SkyManager.rule.judgeLianSan(selectArray,length,MinNum);break;
            case 8:
                return SkyManager.rule.judgeFeijiDaiErDui(selectArray,length,MinNum);break;
            case 9:
                return SkyManager.rule.judgeZhaDan(selectArray,4,MinNum);break;
            case 10:
                return SkyManager.rule.judgeZhaDan(selectArray,5,MinNum);break;
            case 11:
                return SkyManager.rule.judgeZhaDan(selectArray,6,MinNum);break;
            case 12:
                return SkyManager.rule.judgeZhaDan(selectArray,7,MinNum);break;
            case 13:
                return SkyManager.rule.judgeZhaDan(selectArray,8,MinNum);break;
            case 14:
                return SkyManager.rule.judgeWangZha(selectArray);break;
        }
    },
    ...
    judgeCanOutCard: function (type, selectArray, length, num) {
        ...
        if (type == 0) {
            ...
            for (var i = 9; i < 15; i++) {
                tempType = this.judgeISOUTCARD(i, selectArray, length, 2)
                if (tempType) {
                    return i;
                }
            }
            ...
            if(this.judgeISOUTCARD(type,selectArray,length,num,type)){
                return type;
            }
            for (var i = type+1; i < 15; i++) {
                if(this.judgeISOUTCARD(i,selectArray,length,2,type)){
                    return i;
                } 
            }
        } else {
