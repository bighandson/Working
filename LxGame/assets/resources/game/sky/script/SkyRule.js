...
cc.Class({
    ...
    onLoad: function () {
        ...
        SkyManager.rule = this.node.getComponent(LoadGame.getCurrentGame().rule);
        this.initData();
    ...
    update:function()
    {
        if(!SkyManager.isResLoadComplete)
        {
            return;
        }
        if(this.msgBlocked)
    ...
    //     SkyManager.mySeatId = this.seatId;
    ...
    RCMD_Command:function(data) {
        ...
        // cc.log("RCMD_Command",JSON.stringify(data),Function.getName());
        let cmd = data.cmd;
        ...
        } else if (cmd == SkyCommand.RESP_CMD.RCMD_LightStart) {

        } else if (cmd == SkyCommand.RESP_CMD.RCMD_CallLandOwner) {

        } else if (cmd == SkyCommand.RESP_CMD.RCMD_GrabLandOwner) {

        } else if (cmd == SkyCommand.RESP_CMD.RCMD_LightCard) {

        } else if (cmd == SkyCommand.RESP_CMD.RCMD_AddTimes) {
        ...
        else if (cmd == SkyCommand.RESP_CMD.RCMD_Runfirst)
        {
            //
            this.setMsgBlocked(true);
        }
        ...
        else if(cmd == SkyCommand.RESP_CMD.RCMD_Change)
        {
            //收到结果
            this.setMsgBlocked(true);
        ...
        else if(cmd == SkyCommand.RESP_CMD.RCMD_Eastwind)
        {
            //指定借东风
            //this.userarray = retData.userarray;//玩家数据
            this.setMsgBlocked(true);
        ...
        else if(cmd == SkyCommand.RESP_CMD.RCMD_SendPrise)
        {
    ...
    addPomeloListeners:function()
    {
        ...
        for(var i = 0; i < SkyManager.msgRCMDQueue.length; i++){
            pomelo.on(SkyManager.msgRCMDQueue[i],this.msgQueue.bind(this));
        }
    ...
    getFJLianEr: function (pokers,length,num) {
        ...
        var ret =SkyManager.combination(pokers,1);
    ...
    removePomeloListeners : function () {
        for(let i = 0; i < SkyManager.msgRCMDQueue.length; i++){
            pomelo.removeAllListeners(SkyManager.msgRCMDQueue[i]);
        }

        //不需要加入队列的消息
        for(let i = 0; i < SkyManager.msgRCMDList.length; i++){
            pomelo.removeAllListeners(SkyManager.msgRCMDList[i]);
        }
    ...
    {
        return poker.getComponent("SkypokerCtr").weight;
    },

    getPokerTypeByPoker:function(poker)
    {
        return poker.getComponent("SkypokerCtr").type;
    },
    getpokeriofoByPoker:function(poker)
    {
        return poker.getComponent("SkypokerCtr").info;
    },
    ...
    {
        return poker.getComponent("SkypokerCtr").num;
    },
    getPokerNumByPokerS:function(poker)
    {
        return poker.getComponent("SkypokerCtrS").num;
    },

    getPokerIndexByPoker: function(poker)  {
        return poker.getComponent("SkypokerCtr").index;
    },
    ...
    sortHandPokers:function(pokers)
    {
        pokers.sort(function(a,b){
            return a.getComponent("SkypokerCtr").weight-b.getComponent("SkypokerCtr").weight;
        });
    },//
...
////
    ...
    {
        pokers.sort(function(a,b){
            return b.getComponent("SkypokerCtr").weight-a.getComponent("SkypokerCtr").weight;
        });
    },
    sortHandPokersTempML: function (pokers) {
        pokers.sort(function(a,b){
            if(a.getComponent("SkypokerCtr").pokerNum == b.getComponent("SkypokerCtr").pokerNum)
            {
                return b.getComponent("SkypokerCtr").weight - a.getComponent("SkypokerCtr").weight;
            }
            else
            {
                ...
                return b.getComponent("SkypokerCtr").pokerNum - a.getComponent("SkypokerCtr").pokerNum;
            }
        });
    },
        ...
        for(var i = 0;i<tempArr.length;i++)
        {
            ...
            for(var j = 0;j < pokers.length;j++)
            {
                if(temp[0]==this.getPokerNumByPoker(pokers[j]))
                {
                    ...
                    {
                        pokers[j].getComponent("SkypokerCtr").pokerNum = temp[0];
                    }
                    else
                        pokers[j].getComponent("SkypokerCtr").pokerNum = temp[1];
                }
    ...
    {
        pokers.sort(function(a,b){
            return a.getComponent("SkypokerCtr").num - b.getComponent("SkypokerCtr").num;
        });
    },
    ...
    {
        let pokArr=[];//存放牌数组
        let ret =SkyManager.combination(pokers,1);
        let temp = ret[0];
        pokArr.push(temp);
        return pokArr;
    },
    ...
    getDanzhang:function(pokers,length,num,isKing){
        ...
        if(SkyManager.controller.myIsfeiwang ||SkyManager.controller.myshuangfei){
            tempArr=this.createDictionary(pokers,false);
        }else{
            tempArr=this.createDictionary(pokers,true);
        }
    ...
    judgeDanZhang:function(pokers,lenght,num)
    {
        ...
        {
            if (SkyManager.controller.myIsfeiwang ||SkyManager.controller.myshuangfei){
                if (this.getPokerNumByPoker(pokers[0])>15){
                    return false
                }else{
                    return true
                }
    ...
    judgeDuizi:function(pokers,length,num)
    {
        ...
        let tempDZ
        if(SkyManager.controller.myIsfeiwang ||SkyManager.controller.myshuangfei){
            tempDZ=this.getDuizi(pokers,length,num,false);
        }else{
            tempDZ=this.getDuizi(pokers,length,num,true);
        }
        if(tempDZ.length>0)
        {
            return true;
    ...
    getLianSan:function(pokers,length,num)
    {
        ...
        let tempArr=[];//临时数组
        let pokArr=[];//存放牌数组
        if(pokers.length<length||pokers.length<9)
        {
            return pokArr;
        }
        //cc.log("连三---------------------")
    ...
    judgeLianSan:function(pokers,length,num)
    {
        ...
        if(pokers.length!=length||pokers.length%3!=0||pokers.length<9||this.judgeZapai(pokers,pokers.length/3))
        {
            return false;
        }
    ...
    {
        if(pokers.length!=length||pokers.length%5!=0||pokers.length!=15)
        {
            return false;
        }
    ...
    getSiDaiErDui:function(pokers,length,num) {
        ...
        // var tempArr1 = this.createDictionary(pokers);
        // var sanzhangnumArr = this.createDaiCardNumArr(tempArr1, num, 4, false);
        // if (!!sanzhangnumArr.length && sanzhangnumArr.length == 2 && sanzhangnumArr[0][1] == 4 && sanzhangnumArr[1][1] == 4) {
        //     numArr  sanzhangnumArr;
        // } else {
        //     // numArr = this.getsandaiyiArr(tempArr, sanzhangnumArr, 2);
        //     // numArr = this.getSidaiErduiArr(tempArr, sanzhangnumArr, 2);
        //     // getSidaiErduiArr
        //     let listDui = this.getSanDaierArr(pokers,2); // 对子
        //
        ...
        //
        //
        //     for (var i = 0; i < sanzhangnumArr.length; i++) {
        //         let san = numArr[i];
        //
        //
        //         if (listDui.length > 0) {
    ...
    //飞机
    getFeiJiDaiErDui2:function(pokers,length,num){
    ...
    {
        if(pokers.length<length||pokers.length!=15)
        {
            return [];
        }
        ...
        if (listDui.length < numArr[0].length) {
            return [];
        }

        for (var i = 0; i < listDui.length; i++) {
            //如果对子中出现2则直接return
            if(listDui[i][0] == 15){
                return [];
            }else{
                if (i < listDui.length - 1){
                    if (listDui[i+1][0] - listDui[i][0] != 1) {
                        return [];
                    }
                }
    ...
    getsandaiyiArr:function(handArr,Arr,NUM,isKing){

        var Array = [];
        for(var i = 0;i<Arr.length;i++){
            /// var temp = this.deleteSanDAIArr(handArr,Arr[i]);
            var temp1 = Arr[i];
            var Arr1 = this.createDaiCardNumArr(handArr,2,NUM,isKing);
            if(Arr1.length!=0){
                for(var j = 0;j<Arr1.length;j++)
                {
                    var tempArr = [];
    ...
    getSanDaiYidui:function(pokers,length,num,isKing){
        var numArr = [];
        var tempArr = [];
        var Arr = [];
        var pokArr=[];
        ...
        var tempArr1=this.createDictionary(pokers,isKing);
        var sanzhangnumArr=this.createDaiCardNumArr(tempArr1,num,3,false);
        numArr=this.getsandaiyiArr(tempArr,sanzhangnumArr,2);
        var ret =SkyManager.combination(pokers,1);
        for(var i = 0;i<numArr.length;i++)
    ...
    onExit : function () {
        ...
        if(!this.isPlaying){
             ...
             * result 0 取消  1 确定
             */
            showAlertBox('正在游戏中无法退出！');
            //showConfirmBox("正在游戏中无法退出",function(result){
            //    if(result){
            //        // 确定离开
            //        self.exi
            //    }
    ...
    getZhaDan:function(pokers,length,num,isKing){
        ...
        let tempArr=[];//临时数组
        let pokArr=[];//存放牌数组
        if(pokers.length<length || SkyManager.controller.myIsfeizha)
        {
            return pokArr;
        }
    ...
    judgeWangZha:function(pokers)
    {
        ...
        let tempTWZ=this.getWangZha(pokers)
        if(tempTWZ.length>0)
        {
            return true;
        }
