...
const BottomLayout = require('BottomLayout');
const RightLayout  = require('RightLayout');
const RigfhtLayer2  = require('RigfhtLayer2');
const TopLayout    = require('TopLayout');
const LeftLayout   = require('LeftLayout');
...
cc.Class({
    ...
    init : function (chair,count,scale,rule,layout) {
        this.clearForGame()
        this._chair = chair;
        this.scale = scale;
        layout = layout || chair;
        ...
        if(3 == this._chair||0 == this._chair){
          this._hands = this.createMJArray(70-this._chair);
        }else{
          this._hands = this.createMJArray();
        }
        this._handZOrder = 100;

        this._outs = this.createMJArray(50);
        this._outCardZOrder = 100;
        if(chair == 0){
            this._blocks = this.createMJArray(60);
        }else{
        ...
        cc.log('rule.ruleFlag',rule.ruleFlag)
        if(rule.roomType<2){
            this.ruleFlag = Math.pow(2,rule.youxirenshu-2);
        }else{
            this.ruleFlag = rule.ruleFlag
        }
    ...
    _createOneHand : function (value,isGet) {
        ...
        if(this.rule.isBD(value)){
            ...
            mjcard.setBDKuangTag(4,true);
        }

        return mjcard;
    },

    _createOneHandBlock : function (value,isGet,num) {
        isGet = isGet || false;
        let diffPos;
        if(isGet){
            diffPos = this._layout.getLastCardDiff();
        ...
        if(MJType.invalidPai(value)){
            if(!!num){
                frame = MJCardResource.getExtraImageByChair(this._chair,value,2);
            }else{
        ...
        if(this.rule.isBD(value)){
            mjcard.setBDTag(this._chair,false,0,0,1);
            mjcard.setBDKuangTag(this._chair,true);
        }
    ...
    dealActData : function (value,flag,dir) {
        ...
        switch (flag){
            ...
            case 3:
                for(let i = 0; i < 3; i++){
                    let card = this._createOneBlockCard(pais[i],blockNum,i,false,1);
                    if((i+1) == flag){
            ...
            case 5:
                let len = 3 + (flag - 4);
                for(let i = 0; i < len; i++){
                    let card = this._createOneBlockCard(l1,blockNum,i,false,1);
                    if((2 * len - 5) == i){
                        card.setArrowTag(this._chair,dir);
                    }
                }
            ...
            case 6:
                for(let i = 0; i < 4; i++){
                    let card = this._createOneBlockCard(l1,blockNum,i,i!=3,1);
                    if(i == 3){
                        card.setArrowTag(this._chair,dir);
                    }
                }
        ...
        switch (flag){
            ...
            case 6:
                for(let i = 0; i < pais2.length; i++){
                    let card = this._createOneBlockCard(pais2[i],blockNum,i,false,1);
                    if(i == pais2.length-1){
                        card.setArrowTag(this._chair,dir);
                    }
                }
    ...
    dealHitCardsEX :  function (value,opcode,dA,dB,dir) {
        ...
        for(let i = 0; i < 2; i++){
            let card = this._createOneBlockCard(value,blockNum,i,false,1);
            if(i == 1){
                card.setArrowTag(this._chair,dir);
            }
        }
        let card = this._createOneBlockCard(dB,blockNum,2,false,1);
        // 找到真实值
    ...
    dealEatCards : function (value,flag,paiA,paiB,dA,dB,dir) {
        ...
        for(let i = 0; i < 3; i++){
            let card = this._createOneBlockCard(values[i],blockNum,i,false,1);
            if(value == values[i]){
                card.setArrowTag(this._chair,dir);
            }
        }
    ...
    reDrawBlockHandCards : function (pais) {
        ...
        for(let i = 0; i < pais.length; i++){
            let needDiffFlag = (i == pais.length -1 && (i+1) % 3 == 2);
            this._createOneHandBlock(pais[i],needDiffFlag,1);
        }
    },
    dealHitBarCardsEX : function (value,da,db,dc,dir) {
        ...
        let card1 = this._createOneBlockCard(value,blockNum,0,false,1);
        let card2 = this._createOneBlockCard(da,blockNum,1,false,1);
        let card3 = this._createOneBlockCard(db,blockNum,2,false,1);
        let card4 = this._createOneBlockCard(dc,blockNum,3,false,1);
    ...
    dealAnBarCardsEX : function (value,da,db,dc,dd) {
        ...
        let card1 = this._createOneBlockCard(da,blockNum,0,false,1);
        let card2 = this._createOneBlockCard(db,blockNum,1,false,1);
        let card3 = this._createOneBlockCard(dc,blockNum,2,false,1);
        let card4 = this._createOneBlockCard(dd,blockNum,3,false,1);
    ...
    dealAnBarCards : function (value,da,db,dc,dd) {
        ...
        for(let i = 0; i < 4; i++){
            let card = this._createOneBlockCard(value,blockNum,i,i!=3,1);
            if(i==3){
    ...
    dealSupplyBarCards : function (value,da) {
        ...
        let card = this._createOneBlockCard(value,blockNum,3,false,1);
        // card.setArrowTag(this._chair,dir);
        this.removeHandCardIndex(da);
        this.freshHandCardsPos();
    },
    dealSupplyBarCardsEX : function (value,num) {
        ...
        blockNum = num;
        let card = this._createOneBlockCard(value,blockNum,this.blockPais[num].values.length-1,false,1);
        // card.setArrowTag(this._chair,dir);
    ...
    freshHandCardsPos:function(bNotSort){
        ...
        for(let i=0;i<len ;i++){
            ...
            let pos;
            if(this._chair == 0){
                if(this.rule.isBD(mjcard.getValue())){
                    mjcard.setBDTag(this._chair,true);
                }
    ...
    _createOneBlockCard : function (value,blockNum,index,bAnBar,cpg) {
        ...
        if(bAnBar){
          frame = MJCardResource.getAnFrame(this._chair);
            ...
            if(!!cpg){
                frame = MJCardResource.getExtraImageByChair(this._chair,value,1);
            }else{
                frame = MJCardResource.getExtraImageByChair(this._chair,value);
            }
        }
        ...
        if(this.rule.isBD(value)){
            if(this._chair == 0 || this._chair == 2  ){
                mjcard.setBDTag(this._chair,true,0,30);
            }else{
    ...
    _createOneOutCard:function(value){
      ...
      mjcard.setCard(value,frame);
      let len = this._getOutCardsLen();
      let pos = this._layout.getOutPos(len-1,this.ruleFlag,this.rule.paiNum);
      mjcard.setCardOriginPos(pos.x,pos.y);
      let scale = [1,1,1,1];
    //   let scale = [1,1,0.8,1];
    ...
    createMJArray : function (zOrder) {
        ...
        let array = node.addComponent('MJCardArray');
